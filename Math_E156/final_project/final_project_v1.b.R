...
null.boot <- replicate(1000,mean(sample(pairwise_null,10000,replace=T) > sample(pairwise_null,10000,replace=T)))

### use MCMC to calculate area under ECDF
N <- 10^4
x.unif <- runif(N,-1,1)
g <- ecdf(pairwise_obs)
ecdf.eval <- g(x.unif)
