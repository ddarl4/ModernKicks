...
CDF <- ecdf(total); mode(CDF)  #create the cumualtive distribution function
plot(3:18, CDF(3:18), type = "b") #lines are misleading
plot.ecdf(total)
...
abline(v = q, col = "red")
#The quantile function is the inverse of the CDF, except that the CDF is not quite invertible

#For standard probability distributions, R has all the functions you need.
#Suppose that you toss a fair coin 18 times.
plot(0:18,dbinom(0:18, 18, 0.5), type = "h")   #density
plot(0:18,pbinom(0:18, 18, 0.5), pch = 3)
abline(h = deciles, col = "blue")
qbin<-qbinom(deciles, 18, 0.5); qbin
...
qqnorm(weights)
qqline(weights)    #passes throurh 0.25 and 0.75 quantiles

#With skewed data a normal distribution is not a good approximation
#Replicate figure 2.10 a from the text
